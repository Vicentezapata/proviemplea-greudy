...
const talentosEducacionPOST = async (id_usuario, body) => {
  ...
  await sequelize.query(
    ...
    { replacements: { id_talento: talento[0].id_talento, nivel_educacional, carrera: carrera || '', institucion: institucion || '' } }
  );
...
const talentosLaboralPOST = async (id_usuario, body) => {
  ...
  await sequelize.query(
    ...
    { replacements: { id_talento: talento[0].id_talento, empresa, cargo, descripcion: descripcion || '', fecha_inicio: fecha_inicio || null, fecha_fin: fecha_fin || null } }
  );
...
const talentosCompetenciasPUT = async (id_usuario, body) => {
  ...
  for (const id_competencia of competencias) {
    await sequelize.query(
      ...
      { replacements: { id_talento, id_competencia } }
    );
  }
...
const talentosIdiomasPUT = async (id_usuario, body) => {
  ...
  for (const idioma of idiomas) {
    await sequelize.query(
      ...
      { replacements: { id_talento, id_idioma: idioma.id_idioma, nivel_dominio: idioma.nivel_dominio } }
    );
  }
