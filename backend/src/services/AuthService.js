...
const authLoginPOST = async ({ authLoginPostRequest }) => {
  ...
  if (!passwordValida) return { success: false, message: 'Correo o contraseña incorrectos' };
  ...
  try {
    ...
    const [empresaResult] = await sequelize.query(
      `INSERT INTO empresas (rut_empresa, nombre_empresa, id_rubro, id_tipo_empresa)
       VALUES (:rut_empresa, :nombre_empresa, :id_rubro, :id_tipo_empresa)
       RETURNING id_empresa`,
      {
        replacements: {
          ...
          nombre_empresa,
          id_rubro: id_rubro || 1,
          id_tipo_empresa: id_tipo_empresa || 1
        },
        transaction: t
      }
    );
    ...
    await sequelize.query(
      `INSERT INTO usuarios_empresa (id_usuario, id_empresa, nombre_completo)
       VALUES (:id_usuario, :id_empresa, :nombre_completo)`,
      {
        replacements: {
          id_usuario: usuario.id_usuario,
          id_empresa: empresaResult[0].id_empresa,
    ...
    return {
      ...
      data: { id_usuario: usuario.id_usuario, correo: usuario.correo }
    };
